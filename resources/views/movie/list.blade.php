...
<body>
    ...
    <div class="w-[100dvw] h-[100%] grid grid-cols-4 items-center justify-center gap-4 p-[50px]">
        @if ($rec <= 0) <span class="font-bold text-[4rem] col-span-4 row-span-2 m-auto">No movie(s) found for the
            selected filter(s)</span>
            @endif
            @foreach ($mov as $m)
            <div class="m-[15px] card card-side bg-base-100 shadow-xl max-w-[20vw]">
                <figure><img class="max-h-[180px]" src="{{url('storage/'.$m->img)}}"
                        alt="Cover for the movie {{ $m->name }}" /></figure>
                <div class="card-body">
                    <h2 class="card-title">{{ $m->name }}</h2>
                    <p class="text-ellipsis overflow-hidden max-w-[118px]">{{ $m->synopsis }}</p>
